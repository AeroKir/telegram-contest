...
//   const dayMonth = element.columns[0];
//   const newDate = dayMonth.slice(1);
...
//   newDate.sort(getSorted);

//   newDate.forEach((date) => {
//     const dt = getDateAndMonth(date);
//     // console.log(dt);
//   });

//   // console.log(element.columns[0].length);
//   // console.log(element.columns[1].length);
//   // console.log(element.columns[2].length);
// });

// data.forEach((element, i) => {
//   const dayMonth = element[0].columns[0];
//   const newDate = dayMonth.slice(1);
//   newDate.forEach((date) => {
//     const dt = getDateAndMonth(date);
//     console.log(dt);
...
//   const dayMonth = element;
//   const str = `data consist ${i} ${element}`;
//   console.log(str);
//   console.log(dayMonth[i]);

//   // const newDate = dayMonth[1].slice(1);
...
const numbOfFollowers = n[0].columns[1].slice(1);
...
const arrArr = [[10, 40], [20, 15], [30, 78]];
let xAxis = timeScale.length;
let xAxisLeft = timeScale.length;
...
matrix.forEach((el, i) => {
  ...
  point.setAttributeNS(null, 'transform', 'translate(-100, 700) scale(1, -1)');
  ...
  pointLeft.setAttributeNS(null, 'transform', 'translate(-100, 700) scale(1, -1)');
  ...
  line.setAttributeNS(null, 'transform', 'translate(-100, 700) scale(1, -1)');
  ...
  lineLeft.setAttributeNS(null, 'transform', 'translate(-100, 700) scale(1, -1)');
...
// const arrArr = [[1542412800000, 40], [1542499200000, 15], [1542585600000, 78]];
// console.log(arrArr[0][1]);

// console.log(data[0].columns[0]);

import data from './data/chart_data.json';
import firstGraph from './view/chart-view';
...
const testGraph = document.getElementById('test-grid-graph');
testGraph.setAttributeNS(null, 'height', `${yA}`);
testGraph.setAttributeNS(null, 'viewBox', `0 0 400 ${yA}`);
// Create Grid
x.forEach((item, i) => {
  const xAxisLine = document.createElementNS('http://www.w3.org/2000/svg', 'line');
  xAxisLine.setAttributeNS(null, 'x1', item);
  xAxisLine.setAttributeNS(null, 'x2', item);
  xAxisLine.setAttributeNS(null, 'y1', '0');
  xAxisLine.setAttributeNS(null, 'y2', '99%');
  xAxisLine.setAttributeNS(null, 'stroke', 'gray');
  xAxisLine.setAttributeNS(null, 'stroke-width', '1');

  testGraph.appendChild(xAxisLine);
});

y.forEach((item, i) => {
  const yAxisLine = document.createElementNS('http://www.w3.org/2000/svg', 'line');
  yAxisLine.setAttributeNS(null, 'x1', '0');
  yAxisLine.setAttributeNS(null, 'x2', '99%');
  yAxisLine.setAttributeNS(null, 'y1', item);
  yAxisLine.setAttributeNS(null, 'y2', item);
  yAxisLine.setAttributeNS(null, 'stroke', 'gray');
  yAxisLine.setAttributeNS(null, 'stroke-width', '1');

  testGraph.appendChild(yAxisLine);
});
...
arrArr.forEach((eleM, i) => {
  ...
  linePath.setAttributeNS(null, 'd', `M${xPointsArr[i]} ${yPointsArr[i]} L${xPointsArr[i + 1]} ${yPointsArr[i + 1]}`);
  linePath.setAttributeNS(null, 'stroke', 'green');
  linePath.setAttributeNS(null, 'stroke-width', '2');
  linePath.setAttributeNS(null, 'stroke-linecap', 'square');
  linePath.setAttributeNS(null, 'stroke-linejoin', 'miter');
  linePath.setAttributeNS(null, 'fill', 'none');
  linePath.setAttributeNS(null, 'transform', 'translate(0, 490) scale(1, -1)');
  
  testGraph.appendChild(linePath);


  // console.log(testPoint.getScreenCTM());

  // testPoint.getScreenCTM();
});
